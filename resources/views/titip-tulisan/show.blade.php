    ...
    <div class="container-fluid py-5">
        <div class="container py-5">
            <ol class="breadcrumb justify-content-start mb-4">
                ...
                @if ($titipTulisan->category)
                    <li class="breadcrumb-item"><a
                            href="{{ route('news.viewCategory', $titipTulisan->category->slug) }}">{{ $titipTulisan->category->name }}</a>
                    </li>
                @endif
                <li class="breadcrumb-item active text-dark">{{ $titipTulisan->judul }}</li>
            </ol>
            <div class="row g-4">
                <div class="col-lg-8">
                    <div class="mb-4">
                        <h1 class="display-5">{{ $titipTulisan->judul }}</h1>
                        <div class="d-flex mt-3 flex-wrap">
                            <div class="text-dark me-4 mb-2"><i class="fa fa-clock"></i>
                                {{ $titipTulisan->created_at->translatedFormat('d F Y H:i') }}</div>
                            <div class="text-dark me-4 mb-2"><i class="fa fa-eye"></i> {{ $titipTulisan->views ?? 0 }}</div>
                            <div class="text-dark me-4 mb-2"><i class="fa fa-thumbs-up"></i>
                                {{ $titipTulisan->likes->count() }}
                            </div>
                            <div class="text-dark mb-2"><i class="fa fa-user"></i> {{ $titipTulisan->nama_pengirim }}</div>
                        </div>
                    </div>
                    ...
                    <div class="mb-4">
                        <div class="d-flex align-items-center mb-3">
                            ...
                            <div class="d-flex flex-wrap gap-2">
                                <!-- Facebook -->
                                <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url()->current()) }}&quote={{ urlencode($titipTulisan->judul) }}"
                                    target="_blank"
                                ...
                                <!-- Twitter -->
                                <a href="https://twitter.com/intent/tweet?url={{ urlencode(url()->current()) }}&text={{ urlencode($titipTulisan->judul) }}"
                                    target="_blank"
                                ...
                                <!-- WhatsApp -->
                                <a href="https://wa.me/?text={{ urlencode($titipTulisan->judul . ' - ' . url()->current()) }}"
                                    target="_blank"
                                ...
                                <!-- Telegram -->
                                <a href="https://t.me/share/url?url={{ urlencode(url()->current()) }}&text={{ urlencode($titipTulisan->judul) }}"
                                    target="_blank"
                                    class="btn btn-sm btn-primary d-flex align-items-center px-3 py-2 share-btn">
                    ...
                    <div class="position-relative rounded overflow-hidden mb-4">
                        <img src="{{ $titipTulisan->image ? asset('storage/titip-tulisan/' . $titipTulisan->image) : asset('img/noimg.jpg') }}"
                    ...
                    <div class="tab-class mb-5">
                        <div class="d-flex justify-content-between border-bottom mb-4 flex-wrap">
                            <ul class="nav nav-pills d-inline-flex text-center mb-2">
                                ...
                                @if ($titipTulisan->category)
                                    <li class="nav-item mb-3">
                                        <a href="{{ route('news.viewCategory', $titipTulisan->category->slug) }}"
                                            class="d-flex py-2 bg-light rounded-pill active me-2">
                                            <span class="text-dark"
                                                style="width: 100px;">{{ $titipTulisan->category->name }}</span>
                                        </a>
                                    </li>
                                @endif
                            </ul>
                            <div class="d-flex align-items-center mb-2">
                                @auth
                                    <form action="{{ route('titip-tulisan.like', $titipTulisan->id) }}" method="POST" class="like-form">
                                        @csrf
                                        @php
                                ...
                                @else
                                    <a href="{{ route('login') }}" class="btn btn-square">
                                        <i class="far fa-thumbs-up text-primary"></i>
                                    </a>
                                @endauth
                                <span class="ms-1 like-count">{{ $titipTulisan->likes->count() }}</span>
                            </div>
                        </div>
                                ...
                                <div class="row g-4 align-items-center">
                                    <div class="col-3">
                                        <div class="avatar-circle bg-primary text-white d-flex align-items-center justify-content-center rounded-circle"
                                            ...
                                            {{ strtoupper(substr($titipTulisan->nama_pengirim, 0, 1)) }}
                                        </div>
                                    </div>
                                    <div class="col-9">
                                        <h3>{{ $titipTulisan->nama_pengirim }}</h3>
                                        <p class="mb-2">
                                            <i class="fas fa-envelope me-1"></i>{{ $titipTulisan->email_pengirim }}
                                        </p>
                    ...
                    <div class="comments-section bg-white rounded-lg shadow-lg p-5 mt-6">
                        <div class="d-flex align-items-center mb-5 pb-3 border-bottom">
                            <h3 class="h4 fw-bold text-gray-800 mb-0">
                                ...
                                Komentar (<span id="commentsCount">{{ $titipTulisan->comments_count ?? 0 }}</span>)
                            </h3>
                        </div>
                        ...
                        <div class="comment-form mb-6 bg-light rounded p-4 border">
                            ...
                            <form id="commentForm" action="{{ route('comments.store') }}" method="POST">
                                ...
                                <input type="hidden" name="commentable_id" value="{{ $titipTulisan->id }}">
                        ...
                        <div id="commentsList" class="comments-list">
                            @forelse($titipTulisan->comments->whereNull('deleted_at') as $comment)
                                        ...
                                        <div class="flex-grow-1">
                                            <div class="d-flex justify-content-between align-items-start">
                                                <div>
                                                    ...
                                                    @endif
                                                    @if (Auth::check() && Auth::user()->is_admin && $comment->author_email)
                        ...
                        @if ($titipTulisan->comments_count > 5)
                    ...
                    @if ($random->isNotEmpty())
                        <div class="bg-light rounded my-4 p-4">
                            ...
                            <div class="row g-4">
                                @foreach ($random as $randomTitip)
                                    <div class="col-lg-6">
                                        <div class="d-flex align-items-center p-3 bg-white rounded h-100">
                                            <div class="flex-shrink-0">
                                                <img src="{{ $randomTitip->image ? asset('storage/titip-tulisan/' . $randomTitip->image) : asset('img/noimg.jpg') }}"
                                                    class="img-fluid rounded" alt=""
                                                    style="width: 100px; height: 100px; object-fit: cover;">
                                            </div>
                                            <div class="ms-3">
                                                <a href="{{ route('titip-tulisan.show', $randomTitip->slug) }}"
                                                    class="h6 mb-2 d-block">{{ Str::limit($randomTitip->judul, 50, '...') }}</a>
                                                <p class="text-dark mt-2 mb-0">
                                                    <i class="fa fa-clock"></i>
                                                    {{ $randomTitip->created_at->translatedFormat('d F Y') }}
                                                </p>
                                                <p class="text-muted mb-0">
                                                    <i class="fa fa-user"></i> {{ $randomTitip->nama_pengirim }}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-12">
                            <div class="p-3 rounded border">
                                ...
                                @foreach (\App\Models\News::where('status', 'Accept')->latest()->take(5)->get() as $latest)
                                    <div class="mb-3 pb-2 border-bottom">
                                        ...
                                        <small class="text-muted">
                                            <i class="fa fa-clock"></i> {{ $latest->created_at->diffForHumans() }}
                                        </small>
                                    </div>
                                @endforeach
                            </div>
                        ...
                        <div class="col-12">
                            <div class="p-3 rounded border">
                                ...
                                @foreach (\App\Models\TitipTulisan::where('status', 'Accept')->latest()->take(5)->get() as $latestTitip)
                                    <div class="mb-3 pb-2 border-bottom">
                                        <a href="{{ route('titip-tulisan.show', $latestTitip->slug) }}"
                                            class="h6 d-block text-decoration-none">
                                            {{ Str::limit($latestTitip->judul, 40, '...') }}
                                        </a>
                                        <small class="text-muted">
                                            <i class="fa fa-clock"></i> {{ $latestTitip->created_at->diffForHumans() }}
                                        </small>
                                    </div>
                                @endforeach
                            </div>
